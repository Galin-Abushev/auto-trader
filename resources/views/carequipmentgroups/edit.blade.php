<x-layout>
    <x-page-heading>Редакция на група характеристики: {{ $data->group_name}}</x-page-heading>

    <form class="max-w-2xl mx-auto space-y-6" method="POST" action="/carequipmentgroups/{{ $data->id }}">
        @csrf
        @method('PATCH')
        <x-forms.input value="{{ $data->group_name }}" label="Група-характеристика" name="group_name" />
    ...
    <form method="POST" action="/carequipmentgroups/{{ $data->id }}" id="form-delete" class="hiden">
        @csrf
        @method('DELETE')
    </form>
</x-layout>