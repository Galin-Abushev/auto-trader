<x-layout>
    <x-page-heading>Редакция на двигател: {{ $data->engine_name}}</x-page-heading>

    <form class="max-w-2xl mx-auto space-y-6" method="POST" action="/carengines/{{ $data->id }}">
        @csrf
        @method('PATCH')
        <x-forms.input value="{{ $data->engine_name }}" label="Двигател" name="engine_name" />
    ...
    <form method="POST" action="/carengines/{{ $data->id }}" id="form-delete" class="hiden">
        @csrf
        @method('DELETE')
    </form>
</x-layout>