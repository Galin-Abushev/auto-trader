<x-layout>
    <x-page-heading>Редакция на област: {{ $data->region_name}}</x-page-heading>

    <form class="max-w-2xl mx-auto space-y-6" method="POST" action="/regions/{{ $data->id }}">
        @csrf
        @method('PATCH')
        <x-forms.input value="{{ $data->region_name }}" label="Област" name="region_name" />
    ...
    <form method="POST" action="/regions/{{ $data->id }}" id="form-delete" class="hiden">
        @csrf
        @method('DELETE')
    </form>
</x-layout>