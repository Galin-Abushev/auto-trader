<x-layout>
    <x-page-heading>Редакция на характеристика: {{ $data->equipment_name}}</x-page-heading>

    <form class="max-w-2xl mx-auto space-y-6" method="POST" action="/carequipments/{{ $data->id }}">
        @csrf
        @method('PATCH')
        <x-forms.input value="{{ $data->equipment_name }}" label="Характеристика" name="equipment_name" />
    ...
    <form method="POST" action="/carequipments/{{ $data->id }}" id="form-delete" class="hiden">
        @csrf
        @method('DELETE')
    </form>
</x-layout>